...
\begin{itemize}
	\item Menüpunkt \menu[,]{Bearbeiten, Einstellungen}
	\item Tab Netzwerk auswählen
	\item In der Bedienoberfläche (\autoref{fig:get-ip}) wählen Sie den Tab \quote{Netzwerk} aus.
	\item Wählen Sie anschließend den Netzwerkadapter [1] aus, über den die Netzwerkdienste öffentlich gemacht werden.
	\item Die IP-Adresse ist nun im Feld \quote{IPv4 Adresse} [2] zu finden.
\end{itemize}
...
\begin{minipage}{0.9\textwidth}
	\centering
	\captionsetup{type=figure}
	\includegraphics[width=8cm]{presenter/get-ip_de}
	\captionof{figure}{IP-Adresse des eingebetteten Servers}
  	\label{fig:get-ip}
\end{minipage}
...
\begin{itemize}
	...
	\item In der Bedienoberfläche (\autoref{fig:get-ip-broadcast}) wählen Sie den Tab \quote{Live-Stream} aus.
	\item Die IP-Adresse ist nun im Feld \quote{Broadcast Adresse} [1] und die Portnummer im Feld \quote{Port} [2] zu finden.
\end{itemize}
...
\begin{minipage}{0.9\textwidth}
	\centering
	\captionsetup{type=figure}
	\includegraphics[width=8cm]{presenter/get-ip-broadcast_de}
	\captionof{figure}{IP-Adresse des dedizierten Servers}
...
\begin{minipage}{\textwidth}
	\centering
	\captionsetup{type=figure}
	\includegraphics[width=0.45\textwidth]{presenter/client-server-arch}
	\captionof{figure}{Client-Server-Architektur}
...
\begin{enumerate}
	\item Aktivieren Sie \quote{Erweiterte Einstellungen} über das Menü \menu[,]{Ansicht, Erweiterte Einstellungen}, sofern Sie das noch nicht getan haben.
	\item Öffnen Sie die Vortragseinstellungen über das Menü \menu[,]{Bearbeiten, Einstellungen}.
	\item Wählen Sie den Tab \quote{Netzwerk} aus.
...
\item \menu[,]{Dokumente abspeichern}\\Öffnet den Speicherdialog, über den die Folien mitsamt aller Annotationen gespeichert werden können.
\item \menu[,]{Quizergebnisse abspeichern}\\Öffnet den \quote{Quizergebnisse abspeichern}-Dialog.
